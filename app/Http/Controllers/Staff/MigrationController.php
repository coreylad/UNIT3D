...
class MigrationController extends Controller
{
    ...
    /**
     * Test migration with a small batch (10 users) - rolls back, no data committed.
    ...
    public function testMigration(): JsonResponse
    {
        try {
            ...
            $groupMap = array_map('intval', $config['group_map'] ?? []);
            unset($config['group_map']);

            $this->migrationService->testConnection($config);

            // Run inside a transaction that we'll roll back
            $preview = [];
            $logs    = [];
            ...
            try {
                // Migrate 10 users as a test batch
                $result = $this->migrationService->migrateUsers($config, 0, 10, $groupMap);
                $logs   = array_merge($logs, $result['logs'] ?? []);

                if ($result['success']) {
                    // Fetch the inserted users to show preview
                    $preview['users'] = DB::table('users')
                        ->orderByDesc('id')
                        ->limit(10)
                        ->get([
                            'id',
                        ...
                        ->map(function ($user) {
                            $group = DB::table('groups')->where('id', $user->group_id)->value('name');
                            return [
                                'id'         => $user->id,
                                'username'   => $user->username,
                                'email'      => $this->maskEmail($user->email),
                                'group'      => $group ?? "ID:{$user->group_id}",
                                'uploaded'   => $this->formatBytes($user->uploaded),
                                'downloaded' => $this->formatBytes($user->downloaded),
                                'seedbonus'  => number_format((float) $user->seedbonus, 2),
                                'passkey'    => substr($user->passkey, 0, 8) . '...',
                                'legacy'     => $user->legacy ? 'Yes (TSSE auth)' : 'No',
                                'created_at' => $user->created_at,
                            ];
                        })
                        ->toArray();

                    $preview['count']   = $result['count'] ?? 0;
                ...
                // Always roll back - this is just a preview
                DB::rollBack();
            }
            ...
            Log::error('Test migration failed: ' . $e->getMessage());
    ...
    /**
     * Mask email for privacy in preview (show first 2 chars + domain)
     */
