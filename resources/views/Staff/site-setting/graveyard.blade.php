...
@section('title')
    <title>Graveyard Settings - {{ __('staff.staff-dashboard') }} - {{ config('other.title') }}</title>
@endsection

@section('meta')
    <meta name="description" content="Graveyard Settings - {{ __('staff.staff-dashboard') }}" />
@endsection
...
@section('breadcrumbs')
    ...
    <li class="breadcrumb--active">Graveyard Settings</li>
@endsection

@section('page', 'page__staff-site-setting--graveyard')

@section('stylesheets')
    <style>
        .page__staff-site-setting--graveyard { padding: 2rem 0 3rem; }
        .ss-hero {
        ...
        .ss-hero__icon {
            ...
            font-size: 1.5rem; color: #78909C; flex-shrink: 0;
        }
        ...
        .ss-card {
            ...
            border-left: 4px solid #78909C; padding: 3rem;
...
@section('main')
    <div class="ss-hero">
        ...
        <div class="ss-hero__text">
            <h1>Graveyard Settings</h1>
            <p>Configure the torrent resurrection system, seedtime requirements, and rewards.</p>
        </div>
    </div>
    ...
    <form class="form ss-card" method="POST" action="{{ route('staff.site_settings.update') }}">
        @csrf
        @method('PATCH')
        <input type="hidden" name="_section" value="graveyard" />

        <label class="ss-toggle" for="graveyard_enabled">
            <span class="ss-toggle__info">
                <span class="ss-toggle__title">Graveyard System</span>
                <span class="ss-toggle__desc">Enable the torrent resurrection / graveyard system</span>
            </span>
            <label class="ss-switch">
                <input type="hidden" name="graveyard_enabled" value="0" />
                <input id="graveyard_enabled" type="checkbox" name="graveyard_enabled" value="1" @checked(old('graveyard_enabled', $siteSetting->graveyard_enabled)) />
        ...
        <div class="ss-grid">
            <p class="form__group">
                <input id="graveyard_time" class="form__text" name="graveyard_time" type="number" min="0" max="31536000" value="{{ old('graveyard_time', $siteSetting->graveyard_time) }}" placeholder=" " />
                <label class="form__label form__label--floating" for="graveyard_time">Seedtime Required (seconds)</label>
                <span class="form__hint">Time needed for successful resurrection. 2592000 = 30 days</span>
            </p>
            <p class="form__group">
                <input id="graveyard_reward" class="form__text" name="graveyard_reward" type="number" min="0" max="1000" value="{{ old('graveyard_reward', $siteSetting->graveyard_reward) }}" placeholder=" " />
                <label class="form__label form__label--floating" for="graveyard_reward">Freeleech Token Reward</label>
                <span class="form__hint">Number of freeleech tokens awarded for successful resurrection</span>
            </p>
        </div>
