...
<html lang="{{ config('app.locale') }}">
    ...
    @php
        $verifyBackgroundUrl = asset('img/auth/The_Void_Login_Page.png');
        $verifyBackgroundVersion = now()->timestamp;

        foreach (['webp', 'jpg', 'jpeg', 'png', 'gif', 'bmp'] as $extension) {
            $candidate = public_path("img/theme/site-background.{$extension}");

            if (file_exists($candidate)) {
                $verifyBackgroundUrl = asset("img/theme/site-background.{$extension}");
                $verifyBackgroundVersion = filemtime($candidate);
                break;
            }
        }
    @endphp
    <body
        ...
        style="--void-login-bg-image: url('{{ $verifyBackgroundUrl }}?v={{ $verifyBackgroundVersion }}')"
    >
        <main>
